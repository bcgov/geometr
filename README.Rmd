...
```
This returns an `sf` object by default. If you would rather simply return a tibble you can set `as_spatial=FALSE`:
```{r}
geomet_daily_mean(station_number = "10PC003", as_spatial = FALSE)
```

We can also query by time ranges using the `start_date` and `end_date` arguments:
```{r, eval=FALSE}
